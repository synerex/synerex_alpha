...
func isAgentInArea(agentInfo *agent.AgentInfo) bool{
	...
	log.Printf("lat lon is %v, %v, %v, %v, %v, %v, %v, %v\n\n", lat, slat, elat, lon, slon, elon, agentInfo.AgentType.String(), agent.AgentType_name[int32(*agentType)])
...
func setAgent(clt *sxutil.SMServiceClient, dm *pb.Demand){
	...
	log.Printf("set agentInfo is %v, %v\n\n", reflect.TypeOf(dmArgOneof.AgentName), reflect.TypeOf(dmArgOneof.Route))
	...
	opts := &sxutil.SupplyOpts{
		...
		Name: nm, 
		JSON: js, 
		AgentInfo : agentInfo,  // FIX: why rpc error ? AgentName and Route is wrong
	}
...
// nextAreaInfo
	...
	// propose agentInfo
	// select agents in controlled area
...
func getAgents(clt *sxutil.SMServiceClient, dm *pb.Demand){
	...
	if int(dmArgOneof.AreaId) == *areaId && int(dmArgOneof.AgentType) != *agentType{
		...
		nm := "getAgent respnse by car-area-provider"
		js := ""
		opts := &sxutil.SupplyOpts{
			Target: dm.GetId(),
			Name: nm, 
	...
	if simutil.IsSupplyTarget(sp, dmIdList) {
		...
		switch supplyType{
			...
			case "RES_GET_AGENTS":
				...
				//neighborMap := data.AreaInfo.Map.Neighbor
