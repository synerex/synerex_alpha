...
func NewRVO2Route(timeStep float64, globalTime float64, area *area.AreaInfo, agents []*agent.AgentInfo, agentType int64) *RVO2Route {
	r := &RVO2Route{
		TimeStep:   timeStep,
...
func (rvo2route *RVO2Route) CalcDirectionAndDistance(sLat float32, sLon float32, gLat float32, gLon float32) (float32, float32) {
...
func (rvo2route *RVO2Route) Scale() ([]Agent, []Obstacle) {
	// エリア情報
	areaCoord := rvo2route.Area.AreaCoord
...
func (rvo2route *RVO2Route) DecideNextTransit(nextTransit *agent.Coord, transitPoint []*agent.Coord, distance float32, destination *agent.Coord) *agent.Coord {
	// 距離が5m以下の場合
	if distance < 150 {
...
func (rvo2route *RVO2Route) InvScale(scaledAgents []Agent) []*agent.AgentInfo {
	// エリア情報
	areaCoord := rvo2route.Area.AreaCoord
	height := float32(math.Abs(float64(areaCoord.EndLat - areaCoord.StartLat)))
	minLat := float32(math.Min(float64(areaCoord.EndLat), float64(areaCoord.StartLat)))
	...
	for i, scaledAgent := range scaledAgents {
		...
		if simAgent.AgentId == scaledAgent.Id {
			...
			direction, distance := rvo2route.CalcDirectionAndDistance(lat, lon, nextTransit.Lat, nextTransit.Lon)

			var nextCoord *agent.Coord
...
func (rvo2route *RVO2Route) CalcNextAgents() []*agent.AgentInfo {
...
func (rvo2route *RVO2Route) IsAgentInControlledArea(agentInfo *agent.AgentInfo, areaInfo *area.AreaInfo, agentType int64) bool {
...
func (rvo2route *RVO2Route) CalcNextAgentsByRVO() []*agent.AgentInfo {
