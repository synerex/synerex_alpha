...
export default class WorldView extends React.Component {
  ...
  getCoreVector(type, loc) {
    let tv = new THREE.Vector3(
      this.cpw * Math.cos(((Math.PI * 2) / this.coreCount) * type),
      loc,
      this.cpw * Math.sin(((Math.PI * 2) / this.coreCount) * type)
    );
    return tv;
  }
  ...
  onAnimate() {
    ...
    if (this.mcount > MAX_MES_NUM) {
      // we need to update all vectors...
      ...
      for (let i = this.mcount; i < mct; i++) {
        ...
        let fromNodept = new THREE.Vector3(
          lpw * Math.cos(((Math.PI * 2) / this.ndCount) * srcIx),
          tm,
          lpw * Math.sin(((Math.PI * 2) / this.ndCount) * srcIx)
        );
        gm.vertices.push(fromNodept, cv);
  ...
  convertWorldToScreenXY(vec3d) {
    ...
    const xw = this.state.width / 2,
      yw = this.state.height / 2;
    let x = (vv.x + 1) * xw;
    let y = -(vv.y - 1) * yw;
  ...
  render2d() {
    ...
    for (let i = 0; i < this.text2d.length; i++) {
      ...
      if (withline) {
        ...
        ctx.lineTo(v2.x + 30, v2.y - 30);
        ctx.lineTo(v2.x + 30 + ctx.measureText(text).width, v2.y - 30);
        ctx.stroke();
  ...
  componentDidMount() {
    ...
    for (let i = 0; i < this.coreCount; i++) {
      ...
      corelGeom.vertices.push(
        new THREE.Vector3(
          cpw * Math.cos(((Math.PI * 2) / this.coreCount) * i),
          20,
          cpw * Math.sin(((Math.PI * 2) / this.coreCount) * i)
        ),
        new THREE.Vector3(
          cpw * Math.cos(((Math.PI * 2) / this.coreCount) * i),
          -1000,
    ...
    for (let i = 0; i < 180; i++) {
      loopGeom.vertices.push(
        new THREE.Vector3(
          lpw * Math.cos((Math.PI * i) / 90),
          20,
          lpw * Math.sin((Math.PI * i) / 90)
        )
      );
      innerLoopGeom.vertices.push(
        new THREE.Vector3(
          cpw * Math.cos((Math.PI * i) / 90),
          20,
          cpw * Math.sin((Math.PI * i) / 90)
        )
      );
    }
    ...
    for (let i = 0; i < ct; i++) {
      ...
      llgeom.vertices.push(
        new THREE.Vector3(mesh.position.x, 20, mesh.position.z),
...
*/
      ...
      this.nodeNameText[i] = {
        text: "Node:" + i,
        vec: new THREE.Vector3(mesh.position.x, 23, mesh.position.z),
        withline: false
      };
  ...
  render() {
    ...
    if (this.store.getMsgCount() === 0 && this.mgroup !== undefined) {
      // clear button
